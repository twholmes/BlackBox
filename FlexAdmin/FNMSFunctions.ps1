...
###########################################################################
# Registry property return functions

function GetFNMSInstallDir
{
	$dir = (Get-ItemProperty "HKLM:\SOFTWARE\Wow6432Node\ManageSoft Corp\ManageSoft" -name ComplianceInstallDir -ErrorAction Ignore).ComplianceInstallDir
	if (-not $dir) {
		$dir = (Get-ItemProperty "HKLM:\SOFTWARE\ManageSoft Corp\ManageSoft" -name ComplianceInstallDir -ErrorAction Ignore).ComplianceInstallDir
	}
	return $dir
}
