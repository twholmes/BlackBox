...
###########################################################################
# This script provides an interface to perform administrative operations for a BlackBox implementations. 
# It is a wrapper to execute functions provided by different "modules" containing different capabilities
...
$ModulesPath = Join-Path $BlackBoxAdminPath "Modules"
Import-Module -Force (Join-Path $ModulesPath 'BlackBox.psm1')
...
function SetConfigValue([string]$settingName, [string]$value)
{
  ...
  Set-RegKeyValue "HKLM:\SOFTWARE\WOW6432Node\Crayon Australia\BlackBox" $settingName $value  
      ...
      foreach ($s in $mc.Xml.Configuration.Setting) 
      {
        if ($s.Name -eq $settingName) 
        {
          if ($s.Encrypt -and $s.Encrypt.ToLower() -eq "true") 
          {
            ...
            {
              # If an exception occurs while decrypting we assume we have a situation where the encrypted value
              # cannot be decrypted on this machine. In that case we delete the encrypted value, and recursively
              # call GetConfigValue to retrieve a fresh value for the setting.
