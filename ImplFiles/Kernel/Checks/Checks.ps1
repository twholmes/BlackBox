...
function CheckNetworkConnections
{
  ...
  $target = GetConfigValue "BlackBoxServerFQDN"  
...
{
  Log "Checking BlackBox database connectivity"
  $target = GetConfigValue "BlackBoxDBServer"  
  $BlackBoxDBName = GetConfigValue "BlackBoxDBName"
  try 
  {    
    # test database connectivity
    Log "Running Test-SqlDatabase function to test connectivity to <master> on $target"    
    Test-SQLDatabase -Server $target -Database master -UseWindowsAuthentication | Out-Null
    
    # test database connectivity
    Log "Running Test-SqlDatabase function to test connectivity to $BlackBoxDBName on $target"    
    Test-SQLDatabase -Server $target -Database $BlackBoxDBName -UseWindowsAuthentication | Out-Null
  }
  catch 
  {
    Log "Error performing BlackBox database checks: $_" -Level "error"
  }
  finally 
  {
    Log
  }
  Log "Done checking BlackBox database connectivity"
