...
{
  # set logs path if not supplied in params
  $logspath = $LogsPathParam
  if (-NOT ($LogsPathParam))
  {
    $logspath = $ScriptPath + ".log"
  }
  # write the log entry
...
{
  # set results path if not supplied in params
  $resultspath = $ResultsPathParam
  if (-NOT ($ResultsPathParam))
  {
    $resultspath = $ScriptPath + ".txt"
  }
